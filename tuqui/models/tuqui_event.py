...
class TuquiEvent(models.Model):
    ...
    **The queue is the whole point.** Odoo's native outgoing webhook is
    ...
    def init(self):
        """The index the dispatch cron actually needs.

        `state = 'pending' AND next_attempt_at <= now()` ordered by
        `next_attempt_at`, run every minute on every database that installs the
        companion. Two per-field indexes would each answer half of that, which
        is not the same thing.

        And only this one. `state` alone is a prefix of it, so the retention
        sweep and the list view's filters ride it too, and `next_attempt_at` is
        never queried without a state - while every extra index is paid for on
        insert, and a mass action inserts thousands at once.

        Declared here rather than with `index=True` on the fields because a
        composite has no declarative form in 18.0 (19.0 has `models.Index`);
        `create_index` is a no-op when the index already exists, so `init`
        running on every upgrade costs nothing.
        """
        super().init()
        tools.create_index(
            self._cr,
            "tuqui_event_state_next_attempt_idx",
            self._table,
