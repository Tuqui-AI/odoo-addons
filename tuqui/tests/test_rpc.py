...
class TestTuquiRpcGateway(HttpCase):
    ...
    def test_read_only_mode_blocks_writes_and_executes_but_allows_reads(self):
        ...
        including ``read_group``, the grouped read (classified as a read via
        the ``read`` prefix)."""
        ...
        # grouped read works too — read_group is a read, not an execute
        resp = self._rpc(
            "res.partner",
            "read_group",
            args=[[], [], ["is_company"]],
    ...
    def test_read_only_member_allows_the_access_check(self):
        ...
        # today: `check_access_rights` warns inline on 18.0 too, and the runbot
