...
from ..models import (
    tuqui_search as search_module,
    tuqui_search_text as text,
)
from ..models.tuqui_search_config import TuquiSearchConfig
...
class TestTuquiSearchText(BaseCase):
...
@tagged("post_install", "-at_install", "tuqui")
class TestTuquiSearch(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.Config = cls.env["tuqui.search.config"]
        cls.main_company = cls.env.company
        cls.other_company = cls.env["res.company"].create({"name": "Tuqui Search Other Co"})
        cls.restricted_group = cls.env["res.groups"].create({"name": "Tuqui search restricted"})
        cls.user = cls.env["res.users"].create(
            {
                "name": "Tuqui Search User",
                "login": "tuqui_search_user",
        ...
        cls.env["ir.rule"].create(
            {
                "name": "Tuqui search: hide flagged partners",
                "model_id": cls.env.ref("base.model_res_partner").id,
                "groups": [(6, 0, [cls.restricted_group.id])],
                "domain_force": "[('ref', '!=', 'tuqui-hidden')]",
            }
        )
    ...
    def _search(self, query, user=None, model="res.partner", **kwargs):
        ...
        response = env["tuqui.search"].search_relevant(model, query, **kwargs)
    ...
    def _indexed_ids(self, config):
        self.env.cr.execute("SELECT res_id FROM tuqui_search_document WHERE config_id = %s", [config.id])
    ...
    def test_cron_reports_progress_by_batches_and_switches_itself_off(self):
        config = self._enable()
        cron = self.env.ref("tuqui.ir_cron_tuqui_search_index")
        self.assertTrue(cron.active, "enabling a model must wake the cron")
        self.env["ir.config_parameter"].sudo().set_param("tuqui.search.batch_size", "3")
        total = config.backfill_total
        ...
        original = TuquiSearchConfig._backfill_batch
        flush = TuquiSearchConfig._flush_pending_index
        with (
            patch.object(TuquiSearchConfig, "_backfill_batch", autospec=True, side_effect=original) as batches,
            patch.object(TuquiSearchConfig, "_flush_pending_index", autospec=True, side_effect=flush) as flushes,
        ):
            run_job()
    ...
    def test_a_record_that_fails_to_index_does_not_stop_the_batch(self):
        config = self._enable()
        original = TuquiSearchConfig._insert_entries
        ...
        with patch.object(TuquiSearchConfig, "_insert_entries", insert), mute_logger(
            "odoo.addons.tuqui.models.tuqui_search_config"
        ):
            self._index(config)
        indexed = self._indexed_ids(config)
    ...
    def test_records_the_user_cannot_see_never_come_back(self):
        ...
        ranked = {row[0] for row in self.env["tuqui.search"]._rank_page(config, items, items, everything, [], 0, 1000)}
        ...
        self.assertIn(
            ...
            self._ids(admin_env["tuqui.search"].search_relevant("res.partner", "motor siemens", limit=50)),
        )
    ...
    def test_when_the_final_check_removes_candidates_it_asks_for_more(self):
        ...
        self._index(config)
        search_class = type(self.env["tuqui.search"])
    ...
    def test_freshness_never_returns_what_the_user_cannot_see(self):
        ...
        search = self.env(user=self.user)["tuqui.search"]
    ...
    def test_ranking_pages_are_exactly_the_full_ordering(self):
        ...
        self._index(config)
        search = self.env["tuqui.search"]
        partners = self.env["res.partner"].sudo()
        everything = partners._search([]).subselect()
        items, _frequencies, _stats = search._search_items(
        ...
        self.env.cr.execute(
            SQL(
                """
                SELECT res_id, %s, ts_rank_cd('{1,1,1,1}', tsv, %s::tsquery, 33) FROM tuqui_search_document d
    ...
    def test_common_terms_never_hide_records_that_match_them(self):
        ...
        with (
            patch.object(search_module, "CANDIDATE_BUDGET", 5),
            patch.object(search_class, "_rank_page", spy),
        ):
    ...
    def test_rarity_is_counted_inside_the_users_scope(self):
        ...
        _items, admin_df, _stats = search._search_items(
            config, admin_partners, ["pomeloz"], "", admin_partners._search([]).subselect(), []
        )
        _items, user_df, _stats = search._search_items(
    ...
    def test_term_stats_count_only_what_the_user_may_see(self):
        ...
        admin.company_ids = [(4, self.other_company.id)]
        admin_env = self.env(user=admin, context={"allowed_company_ids": [self.main_company.id, self.other_company.id]})
        admin_stats = admin_env["tuqui.search"].search_relevant("res.partner", query)["term_stats"]
        response = self._search(query)
        user_stats = response["term_stats"]
        self.assertEqual(response["term_stats_df_cap"], search_module.DF_CAP)
        ...
        portal = self.env["res.users"].create(
            {
                "name": "Tuqui Search Portal Stats",
                "login": "tuqui_search_portal_stats",
                "group_ids": [(6, 0, [self.env.ref("base.group_portal").id])],
            }
        )
    ...
    def test_words_naming_the_record_type_weigh_least(self):
        ...
        config = self._enable()
        self.assertIn("contact", self.env["tuqui.search"]._type_terms("res.partner", "en_US"))
    ...
    def test_domain_and_limit_are_applied(self):
        ...
        self.assertEqual(
            len(self.env(user=self.user)["tuqui.search"].search_relevant("res.partner", "motor")["records"]),
    ...
    def test_invalid_arguments_are_rejected(self):
        for args in (
            ...
            with self.assertRaises(ValidationError, msg=str(args)):
                self.env(user=self.user)["tuqui.search"].search_relevant(*args)
        ...
        def footprint():
            self.env.cr.execute(
                ...
                SELECT (SELECT array_agg(extname ORDER BY extname) FROM pg_extension),
                       (SELECT count(*) FROM pg_ts_config WHERE cfgname LIKE 'tuqui%%'),
                       (SELECT count(*) FROM pg_proc WHERE proname LIKE 'tuqui%%' OR proname = 'unaccent')
                """
            )
    ...
    def test_project_task_description(self):
        ...
        project = self.env["project.project"].create({"name": "Tuqui search project"})
    ...
    def test_project_task_mercofrio(self):
        ...
        if "project.task" not in self.env:
            self.skipTest("project is not installed")
        self.assertTrue({"task", "project"} <= self.env["tuqui.search"]._type_terms("project.task", "en_US"))
        project = self.env["project.project"].create({"name": "Tuqui search project"})
