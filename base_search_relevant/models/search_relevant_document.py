...
class SearchRelevantDocument(models.Model):
    ...
    * Nothing an admin enables should leave marks on tables other modules own:
    ...
    (see ``search.relevant.config._cron_index``) and by the search itself for what
    ...
    **Nobody reads this model through the ORM.** Its only ACL row grants nothing,
    ...
    rules; the only way it reaches a user is ``search.relevant.search_relevant``,
    which returns ids that went through the user's own ``search`` and fragments
    built from the user's own ``read``.
    """

    _name = "search.relevant.document"
    _description = "Relevance Search Index Entry"
    ...
    layout = fields.Integer(
        readonly=True,
        help="Field layout and normalization this row was built with (search.relevant.config.layout).",
    )
