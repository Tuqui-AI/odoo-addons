...
import { MEDIAS_BREAKPOINTS, SIZES, utils as uiUtils } from "@web/core/ui/ui_service";
...
describe("panel acoplado", () => {
    ...
    test("acoplarse le avisa a Odoo con cuánto lugar se quedó", async () => {
        // La respuesta de Odoo, no una nuestra: `utils.getSize()` sale de las
        // mismas media queries que el servicio.
        const realSize = uiUtils.getSize();
    ...
    test("y también mantiene isSmall, que es un campo aparte", async () => {
        ...
        await animationFrame();
        await animationFrame();
        expect(ui.isSmall).toBe(uiUtils.getSize() <= SIZES.SM);
    });
...
describe("las tres banderas se guardan como un estado, no como tres", () => {
    beforeEach(() => {
        onRpc("tuqui.assistant.sso.nonce", "embed_bootstrap", () => ({
            connected: false,
            base_url: null,
    ...
    test("esconder no guarda un panel cerrado que además está acoplado", async () => {
        after(() => sessionStorage.removeItem("tuqui_panel_state"));
        await mountWithCleanup(MainComponentsContainer);
        const assistant = getService("tuquiAssistant");
        assistant.toggleVisibility();
        await animationFrame();
        assistant.expand();
        await animationFrame();
        ...
        // Acoplado sobre un panel guardado como cerrado es una combinación que no
        // existe en vivo: quien la lea de vuelta tiene que adivinar qué quiso
        // decir, y ya la leen tres lugares distintos.
        const saved = JSON.parse(sessionStorage.getItem("tuqui_panel_state") || "{}");
        expect(saved.panelOpen).toBe(false);
