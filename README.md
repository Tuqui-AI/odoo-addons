...
| `tuqui_assistant` | `18.0`, `19.0` | Panel embebido del asistente Tuqui en el backend de Odoo (chat contextual + propose-then-apply sobre el formulario activo). |
...
| `base_search_relevant` | `19.0` | Búsqueda de texto rankeada sobre los modelos que un administrador activa: índice propio, cron y ranking. Depende sólo de `base` — Tuqui lo consume, pero no hace falta Tuqui. |
